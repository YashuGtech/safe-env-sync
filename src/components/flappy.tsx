...
export function Flappy({ level, objects, levelIndex = 1, devMode = false, resumeGraceSec = 0, initialRunTime = 0, onEnd }: FlappyProps) {
  ...
  useEffect(() => {
    ...
    const eGravity = ULTRA_EASY ? MOON_GRAVITY : EARTH_GRAVITY;
    const eJump = ULTRA_EASY ? level.jump_strength : -8.5;
    const eScroll = ULTRA_EASY ? level.scroll_speed * 0.7 : level.scroll_speed;
    const ePipeGap = ULTRA_EASY ? 240 : FIXED_PIPE_GAP;

    // Tier flags:
    //   1–30   : only spikes / poles / brick pipes (+ coins). No hammers,
    //            blocks, bull, bear, blades, lasers, shooters.
    //   31–50  : untouched (original behaviour).
    //   51–100 : all obstacles, but hammers SHORT + exactly 3 per level,
    //            and rotating blades capped to 4–6 and snapped to edges.
    const TIER_1_30 = levelIndex <= 30;
    const TIER_51_PLUS = levelIndex >= 51;
    const ALLOWED_1_30 = new Set(["spike", "poll", "pipe", "coin"]);
    ...
    const BLOCK_SPACING = 320;
    const HAMMER_PIVOT_Y = 0;
    const HAMMER_HEAD_W = 46;
    const HAMMER_HEAD_H = 26;
    const BLOCK_TILE = 34;
    ...
    const HAMMER_COUNT = TIER_1_30 ? 0 : TIER_51_PLUS ? 3 : (3 + Math.floor(Math.random() * 2));
    ...
    const BLADE_CAP_51 = TIER_51_PLUS ? (4 + Math.floor(Math.random() * 3)) : Infinity;
    let bladeUsed = 0;
    ...
    const BLOCK_S = 44;
    const BLADE_R = 26;
    // Tier 51+: shorter hammer arm.
    const HAMMER_LEN = TIER_51_PLUS ? 54 : 90;
    const SHOOTER_W = 36;
    ...
    const tick = (now: number) => {
      ...
      while (nextIdx < sortedObjs.length && sortedObjs[nextIdx].x_time <= runTime + SPAWN_LEAD_SEC) {
        ...
        // Tier 1–30: keep only spikes, poles, brick pipes and coins.
        if (TIER_1_30 && !ALLOWED_1_30.has(obj.obj_type)) { nextIdx++; continue; }
        // Tier 51+: cap blades to 4–6 and snap to the canvas edges so they
        // never appear inside the pipe/pole gap.
        if (obj.obj_type === "blade" && TIER_51_PLUS) {
          if (bladeUsed >= BLADE_CAP_51) { nextIdx++; continue; }
          bladeUsed++;
          (obj as LevelObject).y = obj.y < 0.5 ? 0.1 : 0.9;
        }
      ...
      // Spawn ahead of the camera so they appear in most levels. Tier 1–30
      // is intentionally hammer-free / block-free (only spikes + poles +
      // brick pipes are allowed).
      const universalSpawnX = W + 80;
      const allowUniversal = !TIER_1_30;
      // Hammers: 3–4 per level on a fixed time schedule.
      while (allowUniversal && hammerCursor < hammerSchedule.length && runTime >= hammerSchedule[hammerCursor]) {
        hammers.push({
          x: universalSpawnX + Math.random() * 60,
          phase: Math.random() * Math.PI * 2,
          period: 1.6 + Math.random() * 0.9,
          // Tier 51+ uses a noticeably shorter hammer arm.
          len: (TIER_51_PLUS ? H * 0.12 : H * 0.22) + Math.random() * (H * 0.04),
        });
        hammerCursor++;
      }
      if (allowUniversal && universalSpawnX - lastBlockSpawnX > BLOCK_SPACING) {
        blocks.push({
          x: universalSpawnX + 40 + Math.random() * 80,
          cols: 3 + Math.floor(Math.random() * 3), // 3-5 wide
          rows: 1 + Math.floor(Math.random() * 3), // 1-3 tall (max ~102px)
        });
